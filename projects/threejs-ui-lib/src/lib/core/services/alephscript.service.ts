...
export class AlephScriptService {
  ...
  private async loadScriptsAndConnect(): Promise<void> {
    ...
    try {
      // Step 1: Load Socket.IO from CDN
      await this.loadScript('socket.io', 'https://cdn.socket.io/4.7.5/socket.io.min.js', () => {
        return typeof (window as any).io !== 'undefined';
      });
      
      // Step 2: Load AlephScript Client from server assets
      await this.loadScript('alephscript', '/assets/alephscript-client.js', () => {
        return typeof (window as any).createAlephScriptClient !== 'undefined';
      });
   ...
   */
  private loadScript(name: string, src: string, verifyFn: () => boolean): Promise<void> {
  ...
  private waitForConnection(timeout: number = 5000): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      const checkConnection = () => {
        if (this.alephClient?.connected || this.alephClient?.socket?.connected) {
          resolve();
